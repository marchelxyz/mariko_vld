...
node export-from-prod.mjs YOUR_IIKO_API_LOGIN

# Или с переменной окружения
IIKO_API_LOGIN=YOUR_IIKO_API_LOGIN node export-from-prod.mjs
```
...
# Обновить через API
curl -X POST "https://your-test-app.example.com/api/db/add-iiko-config?key=CHANGE_ME_DB_ADMIN_ROUTE_SECRET" \
  -H "Content-Type: application/json" \
...
curl "https://your-test-app.example.com/api/db/check-terminal-groups?key=CHANGE_ME_DB_ADMIN_ROUTE_SECRET&restaurantId=zhukovsky"
