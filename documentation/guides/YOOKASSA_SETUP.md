...
- ✅ **Домен с HTTPS** (обязательно для webhook)
  - Есть: `https://your-test-app.example.com`
...
```
shopId: 123456
Секретный ключ: test_your_secret_key_here
```
...
curl -X POST "https://your-test-app.example.com/api/db/add-yookassa-config?key=CHANGE_ME_DB_ADMIN_ROUTE_SECRET" \
  ...
  -d '{
    ...
    "shop_id": "123456",
    "secret_key": "test_your_secret_key_here",
    "callback_url": "https://your-test-app.example.com/api/payments/yookassa/webhook"
  }'
...
5. **"URL для уведомлений":**
   ...
   **Для тестового магазина:**
   ```
   https://your-test-app.example.com/api/payments/yookassa/webhook
   ```
...
curl -X POST "https://your-test-app.example.com/api/payments/yookassa/webhook" \
  -H "Content-Type: application/json" \
  -d '{
    "type": "notification",
...
curl -X POST "https://your-test-app.example.com/api/payments/yookassa/create" \
  -H "Content-Type: application/json" \
  -d "{
...
curl "https://your-test-app.example.com/api/payments/2d8f6a7e-000f-5000-a000-1f77a4d9f1a5"
