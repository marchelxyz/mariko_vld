...
export function BookingForm({ onSuccess }: BookingFormProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {cartItems.length > 0 && (
        <Alert className="bg-mariko-primary/20 border-mariko-primary/30 rounded-[16px]">
          <Info className="h-5 w-5 text-mariko-primary" />
          <AlertDescription className="text-white/90 pl-7">
            <span className="font-semibold text-white">Ваше собранное меню будет передано в ресторан</span> при подтверждении бронирования. Ресторан подготовит ваш заказ к указанному времени.
          </AlertDescription>
        </Alert>
      )}
