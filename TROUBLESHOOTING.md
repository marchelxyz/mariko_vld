...
**Последнее обновление:** 2026-04-02 17:26
...
3. Передавать эту метку не на каждую позицию и не только на первую позицию заказа, а на первую позицию каждого кухонного бакета:
   - `cold`
   - `hot`
   - `bar`
   - `default`
...
- `node --check backend/server/routes/cartRoutes.mjs`
- `git diff --check`
- создать новый смешанный заказ по нескольким цехам и убедиться, что на каждом отдельном чеке цеха под первой позицией печатается `MiniApp: доставка` или `MiniApp: самовывоз`;
- убедиться, что внутри одного чека метка не повторяется под каждой позицией подряд.

**Связанный commit:** `4473ec6` `fix(iiko): добавлены кухонные метки по цехам`
