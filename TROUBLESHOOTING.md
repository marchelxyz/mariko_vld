...
**Последнее обновление:** 2026-04-02 17:02
...
2. Передавать эту метку только один раз, в `comment` первой валидной позиции заказа.
3. Не возвращать старую логику `comment: item.name` для всех позиций, чтобы не вернуть баг с дублированием блюда.

**Проверка:**
- `node --check backend/server/integrations/iiko-client.mjs`
- `git diff --check`
- создать новый `delivery`-заказ и убедиться, что на кухонном чеке под первой позицией печатается `MiniApp: доставка`;
- создать новый `pickup`-заказ и убедиться, что на кухонном чеке под первой позицией печатается `MiniApp: самовывоз`.

**Связанный commit:** `d6e4840` `fix(iiko): добавлена кухонная пометка mini app`
