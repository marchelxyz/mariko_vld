...
- если `tg.initData` в конкретном Telegram wrapper появлялся не сразу, админка считала пользователя админом раньше, чем для server-запросов появлялся signed payload;
- списковые запросы уходили слишком рано, получали `401/403`, а интерфейс молча рисовал `0`, потому что ошибки загрузки не показывались отдельно от пустого списка.
...
4. Добавить короткий retry/polling перед каждым admin server request, чтобы дождаться поздно инициализируемого Telegram `initData`, а не отправлять запрос сразу без signed payload.
5. На страницах `Управление ролями` и `Доступ к доставке` показывать явную ошибку загрузки пользователей и кнопку повтора, а не подменять `401/403` на визуальный `0`.
...
**Связанный commit:** `2645766` `fix(admin): исправлена загрузка списков пользователей в Telegram`

### ❌ Проблема: в Telegram-админке раздел `Управление городами` показывал ошибку `Не удалось загрузить города`, хотя сервер и БД были доступны

**Дата:** 2026-03-29
**Симптомы:**
- в Telegram Mini App при открытии `Управление городами` появлялся alert `Не удалось загрузить города. Проверьте подключение к серверу`;
- при этом сама админ-панель открывалась, а backend оставался доступным;
- ошибка была характерна именно для ранней загрузки страницы после открытия Telegram webview.
...
- убедиться, что список городов загружается без alert;
- при искусственной серверной ошибке убедиться, что alert показывает фактическую причину, а не только текст про подключение.

**Связанные commits:** `7a29ed9` `fix(admin): исправлена загрузка городов в Telegram`; `31df952` `fix(admin): добавлен telegram fallback для загрузки городов`

### ❌ Проблема: в Telegram-админке серверные разделы могли видеть роль `Супер-админ`, но всё равно падали с `Требуется подтверждённая авторизация администратора`

**Дата:** 2026-03-29
**Симптомы:**
- в Telegram Mini App админ-панель открывалась с ролью `Супер-админ`;
- при этом `Управление ролями`, `Доступ к доставке`, `Управление городами`, `Меню`, `Акции` и другие server-backed разделы могли отвечать `401/403`;
- ошибка проявлялась нестабильно и чаще всего сразу после открытия webview.

**Причина:**
- часть admin API использовала `getInitData()` или синхронную сборку auth-заголовков в момент, когда `getPlatform()` ещё временно определялся как `web`;
- в `frontend/src/lib/platform.ts` функция `getInitData()` для платформы `web` не возвращает Telegram payload и фактически смотрит только VK initData;
- из-за этого `webFallbackPlatform: "telegram"` без отдельного Telegram-resolver всё ещё мог остаться без `X-Telegram-Init-Data`;
- дополнительно `frontend/src/shared/api/admin/adminServerApi.ts` ждал initData только для явных платформ `telegram/vk`, поэтому ранние Telegram-запросы уходили без нужной подписи.

**Решение:**
1. В `frontend/src/shared/api/platformAuth.ts` добавить явный Telegram fallback для `webFallbackPlatform: "telegram"`:
   - читать `window.Telegram.WebApp.initData`;
   - читать `tgWebAppData` из URL;
   - читать кэшированные `mariko_tg_init_data` / `mariko_tg_user_id`;
   - уметь парсить Telegram user id из signed initData.
2. Перевести admin-модули `menuApi`, `promotionsApi` и `recommendedDishesApi` на `buildPlatformAuthHeadersAsync(..., { webFallbackPlatform: "telegram" })`.
3. В `frontend/src/shared/api/admin/adminServerApi.ts` для всех не-VK запросов ждать и читать именно Telegram initData, даже если ранний runtime ещё видит платформу как `web`.

**Проверка:**
- открыть Telegram Mini App и последовательно проверить `Управление ролями`, `Доступ к доставке`, `Управление городами`, `Управление меню` и `Управление акциями`;
- убедиться, что разделы загружаются без ошибки `Требуется подтверждённая авторизация администратора`;
- проверить, что в VK-версии поведение не изменилось, так как VK-ветка auth-логики не трогалась.

**Связанный commit:** `25c8cc3` `fix(admin): исправлена telegram-авторизация server-разделов`

### ❌ Проблема: Telegram-админка периодически теряла авторизацию из-за строгой проверки `auth_date` в `initData`

**Дата:** 2026-03-29
**Симптомы:**
- в Telegram Mini App роль в шапке могла оставаться `Супер-админ`, но server-backed разделы падали с `Требуется подтверждённая авторизация администратора`;
- проблема повторялась волнами на одном и том же аккаунте без явной смены роли;
- после фронтовых правок по заголовкам часть пользователей всё равно периодически ловила `401`.

**Причина:**
- backend в `verifyTelegramInitData` жёстко отклонял payload при `auth_date` старше `24h` (дефолт);
- для части клиентов Telegram (особенно desktop/долгие сессии) валидно подписанный `initData` мог приходить со "старой" датой;
- в результате подпись была корректной, но авторизация резалась только из-за TTL, что визуально выглядело как "пропали права админа".

**Решение:**
1. В `backend/server/utils/telegramAuth.mjs` добавить детализированную проверку `initData` с причинами отказа (`invalid_hash`, `expired`, `missing_user` и т.д.).
2. В `backend/server/services/adminService.mjs` для admin-авторизации использовать безопасный fallback:
   - сначала строгая проверка;
   - если не прошла, повторная проверка с расширенным окном по `auth_date` через `maxAgeSeconds`;
   - fallback разрешается только при валидной подписи, то есть подделанные payload остаются запрещены;
   - для fallback использовать отдельный лимит `TELEGRAM_ADMIN_RELAXED_INIT_DATA_MAX_AGE_SECONDS` (по умолчанию `30 дней`), чтобы не открывать бессрочный replay.
3. Добавить server-side логи причин `401`, чтобы будущая диагностика не была "вслепую".

**Проверка:**
- `node --check backend/server/utils/telegramAuth.mjs`
- `node --check backend/server/services/adminService.mjs`
- `npm exec --prefix frontend tsc --noEmit --pretty false`
- `git diff --check`
- в Telegram Mini App проверить server-backed разделы админки сразу после открытия и на "долгой" сессии:
  - `Управление ролями`
  - `Доступ к доставке`
  - `Управление городами`
  - `Управление меню`
...
**Связанные commits:** `faf3786` `fix(admin): добавлен fallback для просроченного telegram initData`; `8a18904` `fix(admin): расширено окно auth_date для telegram админки`

### ❌ Проблема: кросс-платформенная авторизация админки могла дрейфовать между TG и VK из-за эвристики платформы, а не по факту signed payload

**Дата:** 2026-03-29
**Симптомы:**
- в TG и VK админка могла вести себя нестабильно: роль видна, но server-backed разделы иногда отвечают `401/403`;
- поведение зависело от раннего старта webview и момента определения платформы;
- проблема воспроизводилась волнами и выглядела как "слетают права".

**Причина:**
- часть фронтовых запросов выбирала платформу по `getPlatform()` и fallback `telegram`, а не по фактическому наличию подписанного `initData` конкретной платформы;
- для `web`-состояния до инициализации SDK это давало неверную ветку заголовков;
- в `vkCore.isInVk()` была небезопасная эвристика `hostname.includes("vk")`, создававшая ложные срабатывания VK-контекста без реального launch payload.

**Решение:**
1. В `frontend/src/shared/api/platformAuth.ts` добавить режим `webFallbackPlatform: "auto"`:
   - определять платформу по signed payload и runtime hints отдельно для TG и VK;
   - выбирать эффективную платформу для заголовков (`X-Telegram-*` / `X-VK-*`) без смешивания.
2. Перевести auth-вызовы на `webFallbackPlatform: "auto"` в API-слоях (`admin`, `cities`, `menu`, `promotions`, `recommended`, `cart`, `profile`, `booking`, `delivery-access`, `onboarding`).
3. В `frontend/src/lib/vkCore.ts` убрать эвристику определения VK по домену и оставить только:
   - наличие VK WebApp API;
   - наличие VK launch params (`vk_*`, `sign`).
4. В `frontend/src/shared/api/admin/adminServerApi.ts` строить заголовки через единый `buildPlatformAuthHeadersAsync(..., { webFallbackPlatform: "auto" })`.

**Проверка:**
- `npm exec --prefix frontend tsc --noEmit --pretty false`
- `git diff --check`
- в TG и VK mini app проверить одинаковый набор server-backed разделов:
  - `Управление ролями`
  - `Доступ к доставке`
  - `Управление городами`
  - `Управление меню`
  - `Управление акциями`
- убедиться, что права не "слетают" при повторном открытии/долгой сессии.

**Связанный commit:** `2a7d51c` `fix(auth): стабилизирована кросс-платформенная авторизация tg и vk`
...
### ❌ Проблема: в акциях не было предупреждения о слишком большом файле и нельзя было удалить лишнее фото из библиотеки

**Дата:** 2026-03-26
**Симптомы:**
- при выборе большого файла админка не предупреждала заранее о лимите размера и пыталась отправить файл на сервер;
- пользователю было непонятно, почему upload падает, пока не приходила server-side ошибка;
- в модалке `Выбор фото` не было действия удаления, поэтому случайно загруженные изображения навсегда оставались в библиотеке;
- удалить файл из интерфейса и одновременно из Yandex Storage было невозможно.

**Причина:**
- в `PromotionsManagement` отсутствовала явная клиентская проверка `file.size` до запуска upload flow;
- библиотека изображений для акций умела только выбирать и загружать файлы, но не использовала уже готовый `DELETE /storage/:key` backend route;
- общая `ImageLibraryModal` не имела опционального сценария удаления с подтверждением.

**Решение:**
1. В `frontend/src/features/admin/promotions/PromotionsManagement.tsx` добавить клиентский лимит `10 MB` с toast-сообщением:
   - `Файл больше допустимого размера`
   - `Загрузите файл до 10 MB.`
2. Показывать тот же лимит в интерфейсе рядом с кнопками загрузки.
3. В `frontend/src/features/admin/menu/ui/ImageLibraryModal.tsx` добавить опциональную кнопку удаления у карточки изображения.
4. Для акций использовать confirm dialog перед удалением и вызывать `DELETE /storage/:key` через `frontend/src/shared/api/promotionsApi.ts`.
5. После удаления:
   - убирать файл из библиотеки;
   - удалять его из Yandex Storage;
   - если файл уже был привязан к карточкам текущего города, очищать `imageUrl` в этих карточках и требовать последующего `Сохранить`.

**Проверка:**
- `cd frontend && npm exec tsc --noEmit --pretty false`
- `git diff --check`
- ручная проверка в админке:
  1. выбрать файл больше `10 MB` и убедиться, что появляется предупреждение без попытки загрузки;
  2. загрузить обычное фото и убедиться, что оно попадает в библиотеку;
  3. открыть `Выбрать из библиотеки`, нажать удаление у файла и подтвердить действие;
  4. убедиться, что файл исчезает из библиотеки и больше не доступен по старому storage key;
  5. если файл был прикреплён к карточке текущего города, убедиться, что поле картинки очищается и после `Сохранить` ссылка не возвращается.

**Связанный commit:** `c49dd62` `fix(promotions): добавлены лимит файла и удаление из библиотеки`

### ❌ Проблема: удаление фото из библиотеки акций не работало, а управление библиотекой дублировалось в режиме выбора для карточки

**Дата:** 2026-03-26
**Симптомы:**
- при подтверждении удаления в библиотеке акций всплывал toast `Не удалось удалить фото`;
- файл оставался в библиотеке и не удалялся из Yandex Storage;
- на странице `Управление акциями` была отдельная кнопка загрузки в библиотеку, а внутри модалки выбора для карточки одновременно были и upload, и delete, из-за чего библиотека смешивалась с режимом выбора;
- в карточке акции пользователь видел лишние действия управления библиотекой, хотя там нужен был только выбор изображения.

**Причина:**
- frontend удалял файл через `DELETE /storage/:key`, где storage key содержал вложенные сегменты вида `promotions/city-.../...`;
- на продовом пути через nginx/base path такой delete по path-param был ненадёжным для вложенных ключей и не доходил до backend как ожидаемый delete конкретного файла;
- одна и та же модалка использовалась сразу как:
  - менеджер библиотеки;
  - селектор изображения для карточки;
  из-за этого в режиме выбора карточки отображались лишние кнопки управления библиотекой.

**Решение:**
1. Перевести удаление изображения на query-формат `DELETE /storage?key=...`, чтобы ключ файла передавался без зависимости от path-segment routing.
2. В `backend/server/routes/storageRoutes.mjs` добавить поддержку `DELETE /storage?key=...`, сохранив существующую delete-логику удаления из Yandex Storage.
3. На странице `Управление акциями` заменить кнопку `Загрузить фото в библиотеку` на `Открыть библиотеку изображений`.
4. Разделить режимы модалки библиотеки:
   - `Библиотека изображений`:
     - можно загружать;
     - можно удалять;
   - `Выбор фото` для карточки:
     - можно только выбрать изображение;
     - без upload/delete, чтобы не было дублирования.

**Проверка:**
- `node --check backend/server/routes/storageRoutes.mjs`
- `cd frontend && npm exec tsc --noEmit --pretty false`
- `git diff --check`
- ручная проверка в админке:
  1. нажать `Открыть библиотеку изображений` и убедиться, что внутри есть `Загрузить` и удаление;
  2. удалить изображение из библиотеки и убедиться, что toast ошибки не появляется;
  3. повторно открыть библиотеку и убедиться, что файл исчез;
  4. открыть `Выбрать из библиотеки` у карточки акции и убедиться, что там больше нет upload/delete, а остался только выбор картинки.

**Связанный commit:** `48a16c2` `fix(promotions): исправлено удаление и управление библиотекой`
...
3. Добавить:
   ...
   - `CORS_ALLOWED_ORIGINS=https://tg.marikorest.ru,https://vk.marikorest.ru,https://mariko-vld.vercel.app,https://mariko-vld-vk.vercel.app,https://your-timeweb-app.example.com,https://apps.vhachapuri.ru,https://vk.com,https://m.vk.com,https://ok.ru`
...
- в test backend логах повторялось:
  - `[CORS] Origin отклонен: https://your-test-app.example.com`
...
curl -i -X OPTIONS \
  -H 'Origin: https://your-test-app.example.com' \
  -H 'Access-Control-Request-Method: PATCH' \
  -H 'Access-Control-Request-Headers: content-type,x-vk-id,x-vk-init-data' \
  https://your-test-app.example.com/api/cart/admin/users/110044092
...
**Связанный commit:** `71b13ea` `fix(iiko): добавлена поддержка обязательных модификаторов блюд`
...
**Связанный commit:** `e631d1a` `fix(auth): стабилизирована авторизация tg/vk и ошибки городов`

### ❌ Проблема: в «Управление городами» кнопки действий уезжали за правый край на мобильном экране

**Дата:** 2026-03-29
**Симптомы:**
- в карточках городов/ресторанов часть кнопок действий (редактирование, скрытие, удаление) обрезалась справа;
- при расширении окна кнопки появлялись, но на телефоне расширить окно нельзя;
- визуально создавалось ощущение «пропавших» кнопок в Telegram Mini App.

**Причина:**
- в `frontend/src/features/admin/cities/CitiesManagement.tsx` заголовок города и строка ресторана были в одной горизонтальной flex-строке;
- на узкой ширине текст + набор фиксированных action-кнопок не помещались одновременно и уходили за пределы доступной ширины.

**Решение:**
1. Для заголовка города включена адаптивная компоновка:
   - на мобильном (`< sm`) блок стал `flex-col`;
   - кнопки переносятся на отдельную строку и выравниваются вправо.
2. Для строки ресторана включена адаптивная компоновка:
   - на мобильном иконка+текст и кнопки разделены на отдельные ряды;
   - action-кнопки всегда остаются в видимой области.
3. На `sm+` сохранено прежнее поведение с горизонтальной компоновкой.

**Проверка:**
- `npm exec --prefix frontend tsc --noEmit --pretty false`
- открыть `Админ-панель -> Управление городами` в мобильном окне mini app;
- убедиться, что кнопки:
  - у города (`скрыть/показать`, `удалить`);
  - у ресторана (`редактировать`, `🎯`, `скрыть/показать`);
  полностью видимы без горизонтального скролла и без расширения окна.

**Связанный commit:** `ec889f7` `fix(admin): исправлен мобильный overflow кнопок в управлении городами`
...
### ❌ Проблема: на кухонном чеке не было короткой пометки, что заказ пришёл из Mini App и относится к доставке или самовывозу

**Дата:** 2026-04-02
**Симптомы:**
- на кухонном чеке iikoFront кухня видела только стандартные строки вроде `Стол / Зал / Гостей`, без явной пометки источника заказа;
- повару было неочевидно, что заказ относится к `delivery` или `pickup` из Mini App;
- order-level комментарий заказа не решал проблему именно на кухонной ленте;
- при этом ранее уже был баг, когда комментарий каждой позиции дублировал название блюда;
- при печати заказа в несколько цехов одной пометки на первой позиции всего заказа было недостаточно: отдельный чек другого цеха мог остаться без метки;
- когда метка печаталась на первой позиции цехового чека, это могло выглядеть так, будто только первое блюдо относится к доставке/самовывозу.

**Причина:**
- печатная форма кухни не выводила наши `orderType/sourceKey` как заметную метку;
- после фикса `0db4d9a` backend вообще перестал отправлять item-comment, чтобы убрать дубль названия блюда, и на кухонном чеке исчезла любая машинная пометка источника.

**Решение:**
1. В `backend/server/integrations/iiko-client.mjs` добавить короткую кухонную метку:
   - `MiniApp: доставка` для `delivery`;
   - `MiniApp: самовывоз` для `pickup`.
2. Дотянуть `category_name` до order item при серверной гидрации заказа в `backend/server/routes/cartRoutes.mjs`.
3. Передавать эту метку не на каждую позицию и не только на первую позицию заказа, а на последнюю позицию каждого кухонного бакета:
   - `cold`
   - `hot`
   - `khinkali`
   - `bakery`
   - `bar`
   - `default`
4. Для Жуковского определять бакет не только по `category_name`, но и по `item.name`, потому что:
   - хинкали лежат внутри категории `Горячее`;
   - выпечка вынесена в отдельную категорию;
   - отдельные цеха в iikoFront не всегда совпадают 1:1 с нашими menu categories.
5. Не возвращать старую логику `comment: item.name` для всех позиций, чтобы не вернуть баг с дублированием блюда.
6. Не относить всю категорию `Десерты` к `cold` по умолчанию:
   - иначе kitchen-label может уехать на отдельный десертный/выпечной чек;
   - для bakery station учитывать и `Десерты`, и item-patterns вроде `штрудель`, `чизкейк`, `наполеон`, `медовик`.

**Проверка:**
- `node --check backend/server/integrations/iiko-client.mjs`
- `node --check backend/server/routes/cartRoutes.mjs`
- `git diff --check`
- создать новый смешанный заказ по цехам `холодный`, `горячий`, `хинкали`, `выпечка` и убедиться, что на каждом отдельном чеке цеха под последней позицией печатается `MiniApp: доставка` или `MiniApp: самовывоз`;
- отдельно проверить кейс `салат + соус + холодная закуска + десерт` и убедиться, что метка остаётся на холодном чеке, а не уезжает на десерт;
- убедиться, что внутри одного чека метка не повторяется под каждой позицией подряд.

**Связанные commits:** `cd9a226` `fix(iiko): уточнено распределение кухонных цехов`; `d489a07` `fix(iiko): разделены квартира и подъезд и уточнены кухонные цеха`
...
### ❌ Проблема: в комментарии заказа iiko между меткой оплаты и пользовательским текстом появлялась пустая строка

**Дата:** 2026-04-01
**Симптомы:**
- в карточке заказа iiko комментарий отображался как три строки:
  - `ОПЛАТА: ...`
  - пустая строка
  - комментарий пользователя;
- в кратком окне комментария из-за этого пользовательский текст был хуже виден;
- если пользовательский комментарий состоял только из пробелов, в iiko всё равно мог оставаться лишний перенос.

**Причина:**
- в `backend/server/integrations/iiko-client.mjs` комментарий собирался через шаблон `${paymentMethodLabel}\n\n${order.comment}`;
- между системной меткой оплаты и пользовательским текстом всегда вставлялись два перевода строки вместо одного;
- пользовательский комментарий не триммировался перед сборкой итоговой строки.

**Решение:**
1. Перед сборкой итогового комментария триммировать `order.comment`.
2. Склеивать метку оплаты и пользовательский комментарий через один `\n`, а не через пустую строку.
3. Если пользовательский комментарий пустой после trim, отправлять в iiko только системную строку оплаты.

**Проверка:**
- `node --check backend/server/integrations/iiko-client.mjs`
- оформить заказ с пользовательским комментарием и убедиться, что в iiko комментарий идёт так:
  - `ОПЛАТА: ...`
  - `Комментарий пользователя`
- оформить заказ без комментария и убедиться, что остаётся только строка оплаты без пустых строк.

**Связанный commit:** `2174641` `fix(cart): оставлена только онлайн-оплата и улучшено оформление доставки`
...
**Связанный commit:** `d489a07` `fix(iiko): разделены квартира и подъезд и уточнены кухонные цеха`
...
### ❌ Проблема: после нажатия `Подтвердить` заказ продолжал висеть у клиента в `В обработке`, а в примечании к адресу путались квартира и подъезд
...
- после временного workaround `deliveryPoint.comment` стал показывать строку `Квартира / подъезд: ...`, из-за чего квартира визуально подставлялась как подъезд;
...
- в checkout для `квартиры` и `подъезда` использовалось одно общее поле, а `deliveryPoint.comment` не различал эти сущности.
...
3. В `frontend/src/features/cart/CartDrawer.tsx` разделить `Квартиру` и `Подъезд` на два отдельных поля и добавить подсказку `Если подъезда нет, можно указать 0`.
4. В `backend/server/routes/cartRoutes.mjs` и `frontend/src/shared/api/cart/cartApi.ts` протянуть `deliveryEntrance` отдельно от `deliveryApartment`.
5. В `backend/server/integrations/iiko-client.mjs` заполнять `deliveryPoint.comment` отдельными строками:
   - `Квартира: ...`
   - `Подъезд: ...`
   чтобы в iikoFront сотрудники не путали одно поле с другим.
...
- проверить `buildIikoDeliveryPayload(...)` и убедиться, что при наличии данных в `deliveryPoint.comment` уходит раздельная структура `Квартира: ...` / `Подъезд: ...`;
- после деплоя оформить новый delivery-заказ с квартирой и проверить:
  ...
  - в iiko заметка к адресу содержит квартиру и подъезд раздельно.

**Связанный commit:** `в работе`

### ❌ Проблема: checkout в Mini App продолжал показывать наличные и карту при получении, хотя рабочий product-scope переведён только на онлайн-оплату, а на мобильном телефоне было неудобно скрывать клавиатуру в форме доставки

**Дата:** 2026-04-03
**Симптомы:**
- в checkout пользователь по-прежнему видел `Наличными при получении`, `Картой при получении` и `Онлайн-оплата`;
- это расходилось с текущим продуктовым решением оставить в приложении только онлайн-оплату;
- на телефоне длинная форма доставки занимала почти весь экран, а явного действия `Скрыть клавиатуру` не было;
- из-за этого заполнение адреса и комментария было неудобным.

**Причина:**
- frontend `CartDrawer` сохранял старый payment selector на три способа оплаты и дефолтил в `cash`;
- backend `cartRoutes` тоже продолжал принимать `cash/card`, поэтому checkout и серверная логика расходились с новым рабочим сценарием;
- в форме оформления не было явного мобильного действия для blur активного поля и скрытия экранной клавиатуры.

**Решение:**
1. В `frontend/src/features/cart/CartDrawer.tsx` оставить только `Онлайн-оплату` как видимый сценарий checkout.
2. Дефолтный `paymentMethod` перевести на `online` и убрать ветки UI для `cash/card`.
3. В `backend/server/routes/cartRoutes.mjs` отклонять любые `cash/card` submit-запросы сообщением `В приложении сейчас доступна только онлайн-оплата.`
4. В `frontend/src/features/cart/CartDrawer.tsx` добавить естественное мобильное поведение без новых кнопок:
   - скрывать клавиатуру по тапу вне поля ввода;
   - проставить `autocomplete` и `enterKeyHint` на поля формы, чтобы мобильная клавиатура вела пользователя по шагам.
5. В `instructions/integrations/iiko/IIKO_NETWORK_CONFIGURATION_RUNBOOK.md` обновить актуальный директорский запрос под текущий scope: без срочного rename payment type, без webhook-эскалации и без немедленного возврата оплаты при получении.
...
- `node --check backend/server/integrations/iiko-client.mjs`;
- `git diff --check`;
- в Mini App открыть checkout и убедиться:
  - видна только `Онлайн-оплата`;
  - клавиатура закрывается по тапу вне поля без отдельной кнопки;
  - submit ведёт только по онлайн-сценарию.

**Связанные commits:** `2174641` `fix(cart): оставлена только онлайн-оплата и улучшено оформление доставки`; `8718e23` `fix(cart): убрано лишнее скрытие клавиатуры в форме доставки`; `a5b403e` `fix(cart): убрана лишняя подпись под онлайн-оплатой`
...
curl -I https://your-test-app.example.com/tg/
curl -I https://your-test-app.example.com/vk/
...
### ❌ Проблема: прод-деплой с поддержкой обязательных модификаторов не поднимался из-за frontend barrel export, а live-меню оставалось без `modifier_groups`

**Дата:** 2026-04-03
**Симптомы:**
- TimeWeb показывал `Ошибка` на деплое коммита `b521501`, а приложение продолжало работать на предыдущем успешном коммите `66e09c7`;
- в логах сборки `vite` падал frontend build:
  - `src/contexts/CartContext.tsx (7:2): "buildModifierSelectionKey" is not exported by "src/shared/utils.ts"`;
- после этого в `prod` Mini App хинкали уже снова были видны в меню, но наружу не отдавались варианты `Жареные / Вареные`;
- проверка `prod` БД показала `0` блюд с непустым `menu_items.modifier_groups`, хотя код поддержки модификаторов уже был написан локально.

**Причина:**
- `CartContext.tsx` импортировал helper'ы через alias `@/shared/utils`, который в сборке резолвился в файл `frontend/src/shared/utils.ts`, а не в `frontend/src/shared/utils/index.ts`;
- новые helper'ы модификаторов были добавлены только в папочный `index.ts`, но не были реэкспортированы из `frontend/src/shared/utils.ts`;
- из-за упавшего деплоя `prod` остался на старом runtime, который не мог отдать `modifierGroups`;
- отдельно от этого live-данные меню в `prod` не были пересобраны после внедрения новой схемы, поэтому даже в БД `modifier_groups` оставались пустыми, пока не был выполнен ручной backfill.

**Решение:**
- добавить реэкспорт модификаторных helper'ов в `frontend/src/shared/utils.ts`, чтобы `vite build` снова собирался;
- заново запушить fix в `prodrepo` и дождаться, пока TimeWeb поднимет `active` на новом `commit_sha`;
- после успешного деплоя выполнить точечный backfill `menu_items.modifier_groups` только для реально опубликованных и уже поддерживаемых блюд;
- по live iiko-аудиту для сети в текущем Mini App совпали только 8 позиций:
  - 4 хинкали в `zhukovsky-хачапури-марико`;
  - 4 хинкали в `kstovo-hachapuri-mariko`;
- сложные позиции с неподдерживаемыми обязательными модификаторами (`Обед удачливого Денди`, `Чай с вареньем комплимент`) в текущем публичном меню не публиковались и поэтому вручную не активировались.

**Проверка:**
- `npm run build` в `frontend` проходит локально без `buildModifierSelectionKey is not exported`;
- TimeWeb `app 152623` должен быть в статусе `active` на коммите `2c9c9b8`;
- запрос `GET https://tg.marikorest.ru/api/menu/zhukovsky-хачапури-марико` должен возвращать у 4 хинкали непустой `modifierGroups` c вариантами `Жареные` / `Вареные`;
- проверка `prod` БД должна показывать 8 строк с `jsonb_array_length(modifier_groups) > 0`:
  - 4 в `zhukovsky-хачапури-марико`;
  - 4 в `kstovo-hachapuri-mariko`;
- новый заказ с хинкали должен требовать выбор варианта приготовления ещё до отправки заказа в iiko.
