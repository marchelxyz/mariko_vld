...
**Последнее обновление:** 2026-04-03 02:07
...
**Связанный commit:** `cd9a226` `fix(iiko): уточнено распределение кухонных цехов`
...
3. В `backend/server/integrations/iiko-client.mjs` отдельно заполнять `deliveryPoint.comment` строкой `Квартира / подъезд: ...`, потому что в части экранов iikoFront сотрудники читают именно это поле, а не только структурированный `flat`.
...
- проверить `buildIikoDeliveryPayload(...)` и убедиться, что при наличии квартиры в `deliveryPoint.comment` уходит отдельная строка с квартирой/подъездом;
...
**Связанные commits:** `2174641` `fix(cart): оставлена только онлайн-оплата и улучшено оформление доставки`; `в работе`
