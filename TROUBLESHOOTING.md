...
**Последнее обновление:** 2026-03-29 14:57
...
2. В `backend/server/services/adminService.mjs` для admin-авторизации использовать безопасный fallback:
   ...
   - если не прошла, повторная проверка с `allowExpired: true`;
...
**Связанный commit:** `faf3786` `fix(admin): добавлен fallback для просроченного telegram initData`
