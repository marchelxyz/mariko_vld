...
## Текущий запрос директору (актуальный scope)

На текущем этапе Mini App работает только с онлайн-оплатой. Наличными и картой при получении в приложении временно не пользуемся, поэтому не просим сейчас срочно менять payment flow под эти сценарии.

Короткий рабочий запрос в iiko/iikoFront сейчас такой:

1. Проверить печатные формы кухни и доставки.
   Нужно, чтобы сотрудникам было понятно, что заказ пришёл из `MiniApp`, и что это `Доставка` или `Самовывоз`, а не только стандартные служебные строки вроде `Стол / Зал / Гостей`.
2. Проверить, как в карточке доставки отображаются адресные примечания.
   Со стороны backend мы передаём квартиру/подъезд отдельно, но в iikoFront нужно убедиться, что сотрудник реально видит эту часть адреса там, где ей удобно пользоваться.
3. Зафиксировать текущую схему как сетевой стандарт для новых точек:
   - delivery-заказы остаются в delivery-модуле iiko;
   - Mini App использует онлайн-оплату;
   - кухня видит понятную пометку источника заказа на чеке.

Что это даст:

- меньше путаницы на кухне и у операторов;
- меньше ручных вопросов по происхождению заказа;
- единый понятный сценарий для следующих ресторанов сети.

Отдельно, не срочно:

- если позже вернём оплату при получении, тогда уже отдельным этапом нужно будет настраивать `Карта при получении` и проверять шаг `Отправить` в delivery workflow.
...
- маппить в iiko способы оплаты, но в текущем product-scope checkout оставлен только для `online`;
