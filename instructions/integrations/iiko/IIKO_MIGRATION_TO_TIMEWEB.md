...
**Цель:** Основной проект на TimeWeb (root@85.198.83.72)

---

## Контекст TimeWeb проекта

- **Сервер:** TimeWeb VPS, root@85.198.83.72
- **Путь проекта:** `/root/HM-projecttt`
- **Backend:** `/root/HM-projecttt/backend/server/cart-server.mjs` на порту 4010
...
mkdir -p /root/HM-projecttt/backend/server/integrations
mkdir -p /root/HM-projecttt/backend/server/workers
```
...
DST=root@85.198.83.72:/root/HM-projecttt/backend/server
...
```bash
ssh root@85.198.83.72
nano /root/HM-projecttt/backend/server/.env
```
...
**Цель:** /root/HM-projecttt на TimeWeb (root@85.198.83.72)
